...
let emit_var env ppf = function
  | Var id -> pp_print_string ppf (String.lowercase_ascii (Env.lookup_cat env id))
  | _ -> failwith "Implementation bug: Emit.emit_var"
  ...
  let rec of_bnf env ppf = function
    ...
    | sdef :: rest -> match sdef.body with
	  [] -> (* if the body is empty, then it's no worth allowing definition, so skip *)
	    of_bnf env ppf rest
	| _ -> let token = "MV" ^ String.uppercase_ascii sdef.cat in
		 List.iter
  ...
  struct
	...
	Var id ->
	  (try let cat = Env.lookup_cat env id in
	     pf ppf "%s_of_%s of %s" this cat (String.lowercase_ascii cat)
	...
	[] -> failwith "Empty syntax definition"
      | sdef :: rest -> match sdef.body with
	    [] when (String.uncapitalize_ascii sdef.cat = sdef.cat) ->
	  ...
	  | [] ->
		 ...
		 emit a type definition, which wraps string *)
	      pf ppf "@[type %s = %s of string@]@\n" (String.lowercase_ascii sdef.cat) sdef.cat;
	      emit_typedef2 env ppf rest
	  | _ ->
	      pf ppf "@[type %s @[= %a@]@]@ "
		(String.lowercase_ascii sdef.cat)
	  ...
	  let ts =
	    List.map
	      (function
               ...
               | _ -> failwith "Implementation bug: Emit.of_judgments")
              jdg.args
          in
	  pf ppf "%s of @[%a@]"
	    jdg.pred
  ...
  struct
	...
	{pred = pred; args = []} -> pp_print_string ppf pred
	  ...
	  with Not_found -> failwith ("emit_jdg: " ^ jdg.pred ^ " not found")
	    | Invalid_argument _ -> failwith ("emit_jdg: arity mismatch for " ^ jdg.pred)
