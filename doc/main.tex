...
\item ユーザパスワードの生成:
\begin{verbatim}
makepasswd --chars 8 --crypt
\end{verbatim}
で表示されるひとつ目がパスワードそのもの，ふたつ目を site-local.scm に登録する．

\item ゲーム定義が変更されたら: checker を make してコピーする．
tools/cgi で make してゲーム定義の html を生成，games/ にコピー
(local の方で試してからやるのがよい．)
\end{itemize}
...
\item 体系をいろいろ書く．特に TypingML6, PolyTypingML6．
\item 規則名も文字列ではなくヴァリアント化？
\item make といっしょに自動テスト
\item gmファイル中に宣言された名前の衝突をチェック．
\item gm ファイルにフォーマット情報も追加できるようにする．QExpについては
 既に可能．
\item 単項マイナスの取扱を OCaml と同じにする．
\item パスワードの生成をシステムでできるようにする．
\item ランキング機能
\end{itemize}
...
\begin{itemize}
\item 規則の TeX 出力(html用・Scheme 経由)は実装済
\end{itemize}
...
\item [$\Rightarrow$] true, false のみ特別扱い．
...
\item 1::2 と true ::2 がマッチしない，という判断が導出できなかった．

任意のパターンと値について，マッチするか，しないかのどちらかは
導出できるだろうか？(パターン変数の linearity 条件はどこでチェックする？
parser なら M-Cons の条件は余計だし，規則であれば，disjoint でない時
にマッチしないことを導出できるようにする必要あり．)
\item [$\Rightarrow$] parser でチェックすることにした．
\end{itemize}
