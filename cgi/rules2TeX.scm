...
(define (While:AOpTerm op a1 a2)
  `(,a1 "\\," ,op "\\," ,a2))
(define While:BOpTerm While:AOpTerm)
(define (While:NotTerm b) `("!" ,b))
(define While:LOpTerm While:AOpTerm)
(define While:COpTerm While:AOpTerm)
...
(define While:AEvalTo EvalML2:EvalTo)
(define While:BEvalTo EvalML2:EvalTo)
(define (While:Exec st1 c st2)
  `(,c "\\mbox{ changes }" ,st1 "\\mbox{to }" ,st2))

(define (While:mv base . args)
  (let-keywords args ((primes "") (suffix #f))
		(mv base suffix primes '(("st" "\\sigma")))))

(define (While:EmptyTerm) "\\cdot")

(define (While:BindTerm st x v)
  (if (equal? st "\\cdot")
      `(,x "=" ,v)
      `(,st "," ,x "=" ,v)))

(define (While:PlusTerm) '+)
(define (While:MinusTerm) '-)
(define (While:MultTerm) '*)
(define (While:LtTerm) "\\mbox{<}")
(define (While:EqTerm) "\\mbox{=}")
(define (While:LeTerm) "\\mbox{<=}")
(define (While:AndTerm) "\\mbox{&&}")
(define (While:OrTerm) "\\mbox{||}")
