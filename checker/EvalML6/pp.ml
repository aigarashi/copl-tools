...
and print_val ppf v = 
  let with_paren_R = with_paren (fun v_up v -> v > v_up) in
	...
	Value_of_int i -> pr ppf "%d" i
      ...
      | CnstrViii(Cnstr c, v1, v2, v3) -> 
	  pr ppf "%s(%a,%a,%a)" c print_val v1 print_val v2 print_val v3
...
(*  let with_paren_L = with_paren (<) 
  and with_paren_R = with_paren (fun v_up v -> v > v_up) in*)
	...
	Value_of_int i -> pr ppf "%d" i
      ...
      | CnstrV(Cnstr c) -> pr ppf "\\%sCnstrVTerm{%s}" g c
      | CnstrVi(Cnstr c, v) -> pr ppf "\\%sCnstrViTerm{%s}{%a}" g c tex_val v
      | CnstrVii(Cnstr c, v1, v2) -> 
	  pr ppf "\\%sCnstrViiTerm{%s}{%a}{%a}" g c tex_val v1 tex_val v2
      | CnstrViii(Cnstr c, v1, v2, v3) -> 
	  pr ppf "\\%sCnstrViiiTerm{%s}{%a}{%a}{%a}" g c tex_val v1 tex_val v2 tex_val v3
